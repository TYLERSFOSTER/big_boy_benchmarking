"""Parent-source loading for the PlateSupport direct-star diagnostic."""
...
@dataclass(frozen=True)
class DirectStarParentSources:
    """Validated parent evidence needed by the direct-star diagnostic."""
...
class DirectStarParentSourceError(ValueError):
    """Raised when the parent gauntlet source cannot support this diagnostic."""


def load_direct_star_parent_sources(
    *,
    parent_gauntlet_source: Path | str,
    repo_root: Path | str,
) -> DirectStarParentSources:
    ...
    if not isinstance(stage_paths, list):
        raise DirectStarParentSourceError(
    ...
    for stage_id, label in required.items():
        if stage_id not in stages:
            raise DirectStarParentSourceError(f"missing {label} readout source")
    ...
    return DirectStarParentSources(
...
def _resolve_stage_binding(
    ...
    if not isinstance(source_files_payload, dict):
        raise DirectStarParentSourceError(f"stage source_files must be a mapping: {path}")
    source_files = {
    ...
    if missing:
        ...
        raise DirectStarParentSourceError(f"required parent source file missing: {missing[0]}")
...
def _schema_metadata(row: dict[str, str], schema_id: str) -> dict[str, object]:
    ...
    raise DirectStarParentSourceError(f"unsupported parent schema id: {schema_id!r}")
...
def _load_parent_target(stage5: ParentSourceBinding) -> dict[str, object]:
    ...
    if rows:
        ...
        if not isinstance(payload_target, dict):
            raise DirectStarParentSourceError("Stage 5 lacks calibrated target")
        target = dict(payload_target)
    if str(target.get("calibration_status")) != "threshold_calibrated":
        raise DirectStarParentSourceError(f"parent target is not calibrated: {target!r}")
    return target
...
def _enforce_dependency_version(dependency_state: dict[str, object]) -> None:
    ...
    if _version_tuple(raw_version) < MINIMUM_STATE_COLLAPSER_VERSION:
        raise DirectStarParentSourceError(
            "direct-star diagnostic requires state_collapser v0.7.2+ pointwise "
...
def _read_json(path: Path) -> dict[str, object]:
    if not path.exists():
        raise DirectStarParentSourceError(f"readout source does not exist: {path}")
    with path.open(encoding="utf-8") as handle:
        payload = json.load(handle)
    if not isinstance(payload, dict):
        raise DirectStarParentSourceError(f"expected JSON object in {path}")
    return payload
...
def _require_under_repo(path: Path, repo_root: Path, label: str) -> None:
    try:
        ...
        raise DirectStarParentSourceError(f"{label} is outside repository: {path}") from exc
