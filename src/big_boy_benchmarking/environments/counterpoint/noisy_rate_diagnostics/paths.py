"""Path helpers for counterpoint noisy-rate contraction diagnostics."""
...
class NoisyRateDiagnosticsEvaluationPaths:
    root: Path
...
def build_noisy_rate_diagnostics_paths(
    artifact_root: Path | str,
    *,
    evaluation_id: str = EVALUATION_ID,
) -> NoisyRateDiagnosticsEvaluationPaths:
    root = Path(artifact_root) / "evaluations" / evaluation_id
    return NoisyRateDiagnosticsEvaluationPaths(
        root=root,
...
def validate_repo_resident_artifact_root(artifact_root: Path | str) -> Path:
    ...
    try:
        ...
        raise ValueError(
            "noisy rate diagnostics artifacts must be repo-resident under "
