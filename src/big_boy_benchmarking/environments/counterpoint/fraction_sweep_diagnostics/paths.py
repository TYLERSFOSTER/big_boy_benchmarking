"""Path helpers for counterpoint contraction fraction sweep diagnostics."""
...
class FractionSweepDiagnosticsEvaluationPaths:
    root: Path
...
def build_fraction_sweep_diagnostics_paths(
    artifact_root: Path | str,
    *,
    evaluation_id: str = EVALUATION_ID,
) -> FractionSweepDiagnosticsEvaluationPaths:
    root = Path(artifact_root) / "evaluations" / evaluation_id
    return FractionSweepDiagnosticsEvaluationPaths(
        root=root,
...
def validate_repo_resident_artifact_root(artifact_root: Path | str) -> Path:
    ...
    try:
        ...
        raise ValueError(
            "fraction sweep diagnostics artifacts must be repo-resident under "
